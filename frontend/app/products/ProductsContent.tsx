...
export default function ProductsContent() {
  ...
  if (loading) {
    return <div className="max-w-7xl mx-auto px-4 py-16 text-center text-black">Loading...</div>;
  }
    ...
    <div className="max-w-7xl mx-auto px-4 py-16">
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        <div className="lg:w-64 space-y-6">
          ...
          <button onClick={clearFilters} className="w-full bg-gray-200 text-black px-4 py-2 rounded-lg hover:bg-gray-300 transition">
            Clear Filters
          </button>
        </div>
            ...
            <>
                  ...
                  return (
                    <div key={product.id} className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition">
                      <div className="p-4">
                        ...
                        <p className="text-blue-600 font-bold text-xl">KSh {product.base_price?.toLocaleString()}</p>
                        <Link href={`/products/${product.slug}`} className="bg-black text-white block text-center py-2 rounded-full mt-4 text-sm hover:bg-gray-800">
                          View Details
                        </Link>
                      </div>
                    </div>
              ...
              {pagination.totalPages > 1 && (
                <div className="flex justify-center gap-2 mt-8">
                  <button onClick={() => goToPage(pagination.page - 1)} disabled={pagination.page === 1} className="px-4 py-2 border rounded-lg disabled:opacity-50 hover:bg-gray-100">Previous</button>
                  <span className="px-4 py-2 text-black">Page {pagination.page} of {pagination.totalPages}</span>
                  <button onClick={() => goToPage(pagination.page + 1)} disabled={pagination.page === pagination.totalPages} className="px-4 py-2 border rounded-lg disabled:opacity-50 hover:bg-gray-100">Next</button>
                </div>
              )}
            </>
          )}
